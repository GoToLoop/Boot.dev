...
File = dict[str, str]
TestCase = tuple[File, str]


def to_string(file: File) -> str:
    return (
...
def test(input: File, expected: str) -> bool:
    print("---------------------------------")
    print("Inputs:")
    print(f"  filename: {input['filename']}")
    print(f"  content: {input['content'][:30]}...")  # Truncate for display
    print(f"  author_first_name: {input['author_first_name']}")
    print(f"  author_last_name: {input['author_last_name']}")
    print(f"Expected:\n{expected}")
    result = file_to_prompt(input, to_string)
    print(f"Actual:\n{result}")
    if result == expected:
        print("Pass")
        return True
